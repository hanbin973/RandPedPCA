...
for (generation in 11:50) {
  parentsA <- selectInd(pop = popA, nInd = 10, trait = 1)
  parentsB <- selectInd(pop = popB, nInd = 10, trait = 2)
  parentsAB <- selectInd(pop = popAB, nInd = 10, trait = selIndex, scale = TRUE, b = selIndexWeights)
...
Matrix::writeMM(obj = pedA,    file = "pedA50op.mtx")
Matrix::writeMM(obj = pedAInv, file = "pedAInv50op.mtx")
Matrix::writeMM(obj = pedLInv, file = "pedLInv50op.mtx")
write.csv(x = data$pedigree$population, file = "popLabel50op.csv", row.names = FALSE)
typeof(data$pedigree)
write.table(data$pedigree,
            file="pedMeta50op.tsv",
            sep="\t",
            row.names = F,
            quote = F)
