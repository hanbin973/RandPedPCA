...
Matrix::writeMM(obj = pedLInv, file = "pedLInv50.mtx")
write.csv(x = data$pedigree$population, file = "popLabel50.csv", row.names = FALSE)
typeof(data$pedigree)
write.table(data$pedigree,
            file="~/git_repos/RandPedigreePCA/datasets/pedMeta50.tsv",
            sep="\t",
            row.names = F,
            quote = F)
...
tail(id10)
idsUnrel <- c(id10, c(7151:7160))
sUnrel <- c(s10, c(NA, NA, NA, NA, 7151, 7151, 7153, 7153, 7155, 7155))
dUnrel <- c(s10, c(NA, NA, NA, NA, 7152, 7154, 7152, 7154, 7156, 7156))

pedUnrel <- pedigree(sUnrel,
         dUnrel,
         idsUnrel)
LInvUnrel <- getLInv(pedUnrel)
Matrix::writeMM(obj = LInvUnrel,
                file= "~/git_repos/RandPedigreePCA/datasets/LInvUnrel.mtx")
...
ids1010 <- c(ids10num, ids10num + maxId)
s1010 <- c(s10num, s10num + maxId)
d1010 <- c(d10num, d10num + maxId)
